...
import { getUserByEmail, getWasteCollectionTask, saveCollectedWaste, saveReward, updateTaskStatus } from "@/utils/db/actions";
import "../globals.css";
...
export default function CollectPage() {
  ...
  useEffect(() => {
    const fecthUserAndTask = async () => {
      setLoading(true);
      try {
        const userEmail = localStorage.getItem("userEmail");
        if (userEmail) {
          const fecthedUser = await getUserByEmail(userEmail);
          if (fecthedUser) {
            setUser(fecthedUser);
          } else {
